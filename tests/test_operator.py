...
def test_prosign_token_has_no_inter_character_gap() -> None:
    ev = build_events("<KN>", OperatorConfig(wpm=20))

    # <KN> = -.--. => five keydowns and four internal element gaps.
    assert [on for on, _duration in ev].count(True) == 5
    assert [on for on, _duration in ev].count(False) == 4


def test_run_on_token_has_no_inter_character_gap() -> None:
    ev = build_events("<UR>", OperatorConfig(wpm=20))

    # <UR> = ..-.-. => six keydowns and five internal element gaps.
    assert [on for on, _duration in ev].count(True) == 6
    assert [on for on, _duration in ev].count(False) == 5
